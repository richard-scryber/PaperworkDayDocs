...
- **HtmlAgilityPack**: Supports the conversion of 'loose' html to valid XHTML that can be processed by the reader
...
**Expression Syntax**:
```
Variables:    {% raw %}{{model.name}}{% endraw %}
Properties:   {% raw %}{{model.user.firstName}}{% endraw %}
Indexing:     {% raw %}{{model.items[0]}}{% endraw %}
Math:         {% raw %}{{price * 1.2}}{% endraw %}
Functions:    {% raw %}{{concat(firstName, ' ', lastName)}}{% endraw %}
Conditionals: {% raw %}{{age >= 18 ? 'Adult' : 'Minor'}}{% endraw %}
```
...
```
Template: <div>{% raw %}{{model.user.name}}{% endraw %}</div>
           │
           ▼
...
- `<img>` → `HTMLImage` → extends `Image` -> extends `ImageBase`
...
4. DataBind(DataContext)
   - Evaluate {% raw %}{{...}}{% endraw %} expressions
   - Populate templates
   - Apply dynamic data
...
Document.DataBind(DataContext)
    ...
    └─→ For each component:
            │
            ├─→ Evaluate {% raw %}{{...}}{% endraw %} in attributes
            │       │
            ...
            │                       └─→ Expression tree
            │
            ├─→ Evaluate {% raw %}{{...}}{% endraw %} in text content
            │
...
For each component:
    ...
    |
    ├─→ Push 'Applied Style' on to currrent Style Stack
    │
    ...
    │       └─→ Build *complete style* from direct and inherited values.
    │
    │    (continue to use the style and process children)
    │
...
The PDFLayoutItems, and associated PDFXOjects and PDFResources begin 'objects', and instruct the writer what to output.
Once an indrirect object is completed and closed, then (and only then) are it's contents written to the base stream and it will be popped from the stack.
...
**Parse Example**:
```
Input: "{% raw %}{{(model.price * 1.2)}}{% endraw %}"
...
// Usage in template:
{% raw %}{{concat(user.firstName, ' ', user.lastName)}}{% endraw %}
```
...
>
> There are other examples in the [Configuration Guide](configuration-structure) that are more complete.
...
**3. Use in Template**:
```html
{% raw %}<div>{{upper(model.name)}}</div>{% endraw %}
```
