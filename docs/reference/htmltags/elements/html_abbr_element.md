...
The `<abbr>` element is an inline semantic HTML element that marks up abbreviations and acronyms. In Scryber, it renders text in italic by default and supports the `title` attribute to provide the full form of the abbreviation. Unlike the standard HTML elements, where `title` appears in the outline, in the library title can be used for accessibility, annotations, or displayed contextually.
...
```html
{% raw %}<div data-bind="{{model.technicalTerms}}">
    <p>
...
- [@title attribute](/reference/htmlattributes/attrs/attr_title)
- [@class attribute](/reference/htmlattributes/attrs/attr_class)
